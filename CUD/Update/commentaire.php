...
$update = "UPDATE `commentaires` SET `valide`= 0 WHERE `idCommentaire` = :idCommentaire AND `id_Sortie` = :id_Sortie";
$insert = new CurDB($update, $parametre);
$action = $insert->actionDB();
header('location:../../index.php?message=Commentaire effacé.&idNav='.$idNav.'&idSortie='.$idSortie);
} else {
