...
$update = "UPDATE `commentaires` SET `commentaire`= :commentaire WHERE `idCommentaire` = :idCommentaire AND `id_Sortie` = :id_Sortie";
$insert = new CurDB($update, $parametre);
$action = $insert->actionDB();
header('location:../../index.php?message=Commentaire modifié.&idNav='.$idNav.'&idSortie='.$idSortie);
} else {
